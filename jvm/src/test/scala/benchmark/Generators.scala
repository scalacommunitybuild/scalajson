...
object Generators {
  def jBoolean: Gen[ast.JBoolean] = for {
  ...
  def jString: Gen[ast.JString] = for {
    size <- Gen.range("seed")(300000, 1500000, 300000)
  } yield {
    ast.JString(size.toString)
  }

  def jNumber: Gen[ast.JNumber] = for {
    size <- Gen.range("seed")(300000, 1500000, 300000)
  } yield {
    ast.JNumber(size)
  }

  def jArray: Gen[ast.JArray] = for {
  ...
  def jObject: Gen[ast.JObject] = for {
    size <- Gen.range("seed")(0, 10, 1)
  ...
  def jValue: Gen[ast.JValue] = Gen.range("JValue type")(300000, 1500000, 300000).flatMap { seed =>
    seed % 5 match {
