...
object Generators {
  ...
  def jBoolean: Gen[unsafe.JBoolean] = for {
  ...
  def jString: Gen[unsafe.JString] = for {
    size <- Gen.range("seed")(300000, 1500000, 300000)
  } yield {
    unsafe.JString(size.toString)
  }

  def jNumber: Gen[unsafe.JNumber] = for {
    size <- Gen.range("seed")(300000, 1500000, 300000)
  } yield {
    scala.json.ast.unsafe.JNumber(size)
  }

  def jArray: Gen[unsafe.JArray] = for {
  ...
  def jObject: Gen[unsafe.JObject] = for {
    size <- Gen.range("seed")(0, 10, 1)
  ...
  def jValue: Gen[unsafe.JValue] = Gen.range("JValue type")(300000, 1500000, 300000).flatMap { seed =>
    seed % 5 match {
